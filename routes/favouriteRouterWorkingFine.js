    ...
    Favourites.find({})
        ...
        .then((favourites) => {
            ...
            if (favourites) {
                ...
                res.json(user_favourites).pretty();
            } else {
...
.post(cors.corsWithOptions, authenticate.verifyUser, 
    (req, res, next) => {
        Favourites.find({}).populate('user').populate('dishes').then((favourites) => {
                var user;
                if(favourites)
            ...
            if(favToRemove){
                favToRemove.remove()
                    .then((result) => {
                        res.statusCode = 200;
                        res.setHeader("Content-Type", "application/json");
                        //res.json(result).pretty();
                        res.send("We have removed all your favourites!")
                    }, (err) => next(err));
    ...
    Favourites.find({})
        ...
        .then((favourites) => {
            if (favourites) {
                ...
                const dish = favs.dishes.filter(dish => dish.id.toString()  === req.params.dishId.toString() )[0];
                if(dish) {
                    res.statusCode = 200;
                    res.setHeader("Content-Type", "application/json");
                    // res.json(dish).pretty();
                    res.send('We have removed the dish with id :' + req.params.dishId + ' from your favourites');
                } else {
                    var err = new Error('You do not have the dish with id provided:' + req.params.dishId);
                    err.status = 404;
                    return next(err);
                }
            } else {
